...
const FilterSVG = ({ filter, clearFilter, setIsFilterOpen }) => {
  const fill = filter ? "fill-active" : "fill-inactive";
  const textColor = filter ? "text-active" : "text-inactive";

  return (
    <div className='cursor-pointer flex justify-center gap-[1vw] items-center'>
      <svg className={`w-[4vw] h-[4vw] md:w-[1.5vw] md:h-[1.5vw] ${fill}`} viewBox="0 0 24 24" aria-hidden="true" focusable="false">
